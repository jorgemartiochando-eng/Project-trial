...
justification, compute the minimum total pay increase that brings the
...
def simulate(df: pd.DataFrame, settings: Settings, target_gap: float | None = None,
    ...
    for (entity, cid, label), grp in df.groupby(["legal_entity", "category_id", "category_label"]):
        ...
        m = grp[grp["sex"] == "M"]
        f = grp[grp["sex"] == "F"]
        g = gap(m["pay_total"], f["pay_total"])
        if g is None or g <= target:
            continue
        justified = bool(settings.justifications.get(key, "").strip())
        if justified and not include_justified:
            continue
        mean_m = m["pay_total"].mean()
        required_mean_f = (1 - target) * mean_m
        budget = (required_mean_f - f["pay_total"].mean()) * len(f)
        inc = _water_fill(f["pay_total"].to_numpy(dtype=float), budget, cap=mean_m)
        annual_cost = 0.0
        for (_, r), di in zip(f.iterrows(), inc):
            if di <= 1e-9:
                continue
            # Implement as a basic salary increase on the annual FTE base salary.
            new_base = r["base_salary"] + di * r["pay_to_base_fte"]
            cost = di * r["pay_to_annual"]
            annual_cost += cost
            adjustments.append({
                ...
                "increase_pct": _clean(di / r["pay_basic"]),
                "annual_cost": _clean(cost),
            })
        new_f = f["pay_total"].to_numpy() + inc
        categories.append({
            "key": key,
            "legal_entity": entity,
            "category_id": cid,
    ...
    total_payroll = float((df["pay_total"] * df["pay_to_annual"]).sum())
